...
<template>
    <div v-if="selectedRelease?.files" class="md:mb-4 px-6 sm:px-10">
        <div class="flex flex-row justify-start items-end gap-1.5 pb-2 mb-3 border-b border-vp-divider">
        ...
        <div v-if="categorizedFiles.development.length > 0" class="mt-3 mb-4">
            <button @click="toggleDevelopmentSection"
                ...
                <v-icon name="oi-chevron-down" scale="0.8" class="transition-transform duration-200 text-vp-2"
                    :class="{ 'rotate-180': isDevelopmentSectionExpanded }" />
                <span class="text-sm text-vp-2 font-medium group-hover:text-vp-1 transition-colors duration-100">{{
                ...
                <div v-if="isDevelopmentSectionExpanded" class="mt-1">
                    <div class="flex flex-col gap-5">
                        ...
                        <div class="flex-1 min-w-80">
                            <div class="mb-3">
                                <FileGrid :files="categorizedFiles.development" :branch="selectedRelease.branch" />
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div
            ...
            <div class="flex flex-row gap-x-1.5 items-center text-sm mr-auto">
                <Tooltip :aria-label="tr('releases_install')" :delay="0" :hide-delay="100" :max-width="'315px'"
                    ...
                    <template #content>
                        <div class="prose prose-sm dark:prose-invert text-vp-1 max-w-none text-left justify-center"
                            v-html="tr('releases_install_tooltip', {
                ...
                <span class="text-vp-1 text-base font-medium py-1 leading-6 tracking-wide whitespace-nowrap">
                    {{ isCurrentVersion ? tr("releases_reinstall") : tr("releases_install") }}
                </span>
            </div>
            <div class="flex flex-col sm:flex-row gap-y-2 gap-x-3 items-end sm:items-center text-[13px] mb-2 sm:mb-0">
                <template v-for="method in installMethods" :key="method.name">
                    <a v-if="method.show" :href="method.href" target="_blank" rel="noopener"
